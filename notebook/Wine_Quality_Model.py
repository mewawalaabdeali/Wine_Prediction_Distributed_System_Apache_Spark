...
lr = LogisticRegression(featuresCol="scaledFeatures", labelCol="quality", maxIter=200,               # Increased iterations
    regParam=0.1,              # Regularization parameter
    elasticNetParam=0.5,       # ElasticNet mixing
...
rf=RandomForestClassifier(featuresCol="scaledFeatures", labelCol="quality", numTrees=21, maxDepth=30, maxBins=32, featureSubsetStrategy="auto", impurity='gini', seed=42)
...
dt = DecisionTreeClassifier(featuresCol="scaledFeatures", labelCol="quality", maxDepth=5)

# COMMAND ----------

dt_model=dt.fit(df_scaled_final)
dt_predictions=dt_model.transform(df_test_final)
dt_predictions.printSchema()

# COMMAND ----------

dt_predictions.select("quality", "prediction", "scaledFeatures").show(10)
...
print(f"Validation Accuracy: {dt_accuracy:.2f}")
print(f"Validation F1 Score: {dt_f1:.2f}")
...
cv_model = cv.fit(df_scaled_final)

# Use the best model to make predictions
predictions = cv_model.transform(df_test_final)

# Evaluate
f1_score = evaluator.evaluate(predictions)
print(f"Best Validation F1 Score: {f1_score:.2f}")
