...
# Logistic Regression
log_reg = LR(labelCol="quality", featuresCol="scaled_features", family="multinomial", maxIter=10, regParam=0.01, elasticNetParam=0.8)

# XGBoost Classifier (using OneVsRest with XGBoost)
...
crossval_xgb = CrossValidator(estimator=xgb_classifier,
                               estimatorParamMaps=paramGrid,
                               evaluator=MulticlassClassificationEvaluator(labelCol="quality", metricName="accuracy"),
                               numFolds=3)
...
pipeline_xgb = Pipeline(stages=[assembler, scaler, crossval_xgb])
...
pipeline_model_xgb = pipeline_xgb.fit(train_data)
...
accuracy_xgb = evaluator.evaluate(pipeline_model_xgb.transform(test_data))

# Step 12: Compare Model Performance and Select the Best Model
best_model = None
model_name = ""

# Compare accuracies
if accuracy_rf > accuracy_dt and accuracy_rf > accuracy_lr and accuracy_rf > accuracy_xgb:
...
elif accuracy_dt > accuracy_rf and accuracy_dt > accuracy_lr and accuracy_dt > accuracy_xgb:
...
elif accuracy_lr > accuracy_rf and accuracy_lr > accuracy_dt and accuracy_lr > accuracy_xgb:
    best_model = pipeline_model_lr
    model_name = "LogisticRegressionModel"
    print(f"Logistic Regression model performs better with accuracy: {accuracy_lr}")
else:
    best_model = pipeline_model_xgb
    model_name = "XGBoostModel"
    print(f"XGBoost model performs better with accuracy: {accuracy_xgb}")
...
# Step 14: Print Model Name for Jenkins
print(f"MODEL_NAME={model_name}_{timestamp}")

# Step 15: Evaluate Model Metrics
