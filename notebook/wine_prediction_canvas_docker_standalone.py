...
s3_model_path = f"s3://winepredictionabdealicanvas/winemodels/{model_folder_name}/"
...
if not os.path.exists(model_dir):
    ...
    try:
        ...
        s3_objects = s3_client.list_objects_v2(Bucket="winepredictionabdealicanvas", Prefix=f"winemodels/{model_folder_name}")
        if 'Contents' in s3_objects and len(s3_objects['Contents']) > 0:
            ...
            for obj in s3_objects['Contents']:
                s3_key = obj['Key']
                local_path = os.path.join(model_dir, os.path.relpath(s3_key, f"winemodels/{model_folder_name}"))
